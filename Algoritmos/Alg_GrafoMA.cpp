...
#include "Stack.hpp"  // requerido por Profundidad2()
#include "Queue.hpp"  // requerido por Anchura()
...
static Lista<GrafoMA::vertice> Profun(const GrafoMA& G, GrafoMA::vertice v,
...
Lista<GrafoMA::vertice> Profundidad2(const GrafoMA& G, GrafoMA::vertice u)
{
   typedef GrafoMA::vertice vertice;
   const size_t n = G.numVert();
   vector<visitas> marcas(n, NO_VISITADO);
...
Lista<GrafoMA::vertice> Anchura(const GrafoMA& G, GrafoMA::vertice u)
{
   typedef GrafoMA::vertice vertice;
   const size_t n = G.numVert();
   vector<visitas> marcas(n, NO_VISITADO);
