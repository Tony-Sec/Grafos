...
#include "../Representaciones/Matrix.hpp"             // para Floyd
#include "Apo.hpp"                                    // para Prim y Kruskall
#include "Partition.hpp"                              // para Kruskall
...
template <typename tCoste> tCoste suma(tCoste x, tCoste y)
{
   const tCoste INFINITO = GrafoPMC<tCoste>::INFINITO;
...
template <typename tCoste>
vector<tCoste> Dijkstra(const GrafoPMC<tCoste>& G,
                        typename GrafoPMC<tCoste>::vertice origen,
                        vector<typename GrafoPMC<tCoste>::vertice>& P)
// Calcula los caminos de coste mínimo entre origen y todos los
...
// de origen a i.
{
   typedef typename GrafoPMC<tCoste>::vertice vertice;
   vertice v, w;
   ...
   for (size_t i = 1; i <= n-2; i++) {
      ...
      tCoste costeMin = GrafoPMC<tCoste>::INFINITO;
      for (v = 0; v < n; v++)
         if (!S[v] && D[v] <= costeMin) {
            costeMin = D[v];
            w = v;
...
template <typename tCoste> typename GrafoPMC<tCoste>::tCamino
camino(typename GrafoPMC<tCoste>::vertice orig,
       typename GrafoPMC<tCoste>::vertice v,
       const vector<typename GrafoPMC<tCoste>::vertice>& P)
// Devuelve el camino de coste mínimo entre los vértices orig e v
// a partir de un vector P obtenido mediante la función Dijkstra().
{
   typename GrafoPMC<tCoste>::tCamino C;
...
template <typename tCoste>
Matrix<tCoste> Floyd(const GrafoPMC<tCoste>& G,
                     Matrix <typename GrafoPMC<tCoste>::vertice>& P)
...
// mínimo de i a j, si este vértice es i el camino es directo.
{
   typedef typename GrafoPMC<tCoste>::vertice vertice;
   const size_t n = G.numVert();
   Matrix<tCoste> A(n);   // matriz de costes mínimos
...
template <typename tCoste> typename GrafoPMC<tCoste>::tCamino
caminoAux(typename GrafoPMC<tCoste>::vertice v,
          typename GrafoPMC<tCoste>::vertice w,
          const Matrix<typename GrafoPMC<tCoste>::vertice>& P)
// Devuelve el camino de coste mínimo entre v y w, exluidos estos,
// a partir de una matriz P obtenida mediante la función Floyd().
{
   typename GrafoPMC<tCoste>::tCamino C1, C2;
   typename GrafoPMC<tCoste>::vertice u = P[v][w];
...
template <typename tCoste> typename GrafoPMC<tCoste>::tCamino
camino(typename GrafoPMC<tCoste>::vertice v,
       typename GrafoPMC<tCoste>::vertice w,
       const Matrix<typename GrafoPMC<tCoste>::vertice>& P)
// Devuelve el camino de coste mínimo desde v hasta w a partir
// de una matriz P obtenida mediante la función Floyd().
{
   typename GrafoPMC<tCoste>::tCamino C = caminoAux<tCoste>(v, w, P);
   C.insertar(v, C.primera());
   C.insertar(w, C.fin());
   return C;
}
...
/*----------------------------------------------------------------------------*/
template <typename tCoste>
GrafoPMC<tCoste> Prim(const GrafoPMC<tCoste>& G)
// Devuelve un árbol generador de coste mínimo
// de un grafo no dirigido ponderado y conexo G.
{
   assert(!G.esDirigido());
   ...
   typedef typename GrafoPMC<tCoste>::vertice vertice;
   typedef typename GrafoPMC<tCoste>::arista arista;
   const tCoste INFINITO = GrafoPMC<tCoste>::INFINITO;
   arista a;
   const size_t n = G.numVert();
   GrafoPMC<tCoste> g(n);      // Árbol generador de coste mínimo.
...
template <typename tCoste>
GrafoPMC<tCoste> Kruskall(const GrafoPMC<tCoste>& G)
// Devuelve un árbol generador de coste mínimo
// de un grafo no dirigido ponderado y conexo G.
{
   assert(!G.esDirigido());
